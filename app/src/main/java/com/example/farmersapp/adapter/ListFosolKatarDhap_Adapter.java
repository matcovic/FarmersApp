...
public class ListFosolKatarDhap_Adapter extends RecyclerView.Adapter<ListFosolKatarDhap_Adapter.ListFosolKatarDhap_ViewHolder> {

    Context mContext;
    List<String> fosolKatarDhap;
    Map<String, CustomListItem_Diseases> diseasesModelMap;

    CustomListItem_Diseases data;
    public ListFosolKatarDhap_Adapter(Context mContext, List<String> fosolKatarDhap, Map<String, CustomListItem_Diseases> diseasesModelMap) {
    ...
    @NonNull
    @Override
    public ListFosolKatarDhap_ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View layout;
        mContext = parent.getContext();
        layout = LayoutInflater.from(parent.getContext()).inflate(R.layout.list_cultivation_disease, parent, false);
        return new ListFosolKatarDhap_ViewHolder(layout);
    }

    @Override
    public void onBindViewHolder(@NonNull final ListFosolKatarDhap_ViewHolder holder, int position) {
         data = diseasesModelMap.get(fosolKatarDhap.get(position));
        assert data != null;
    ...
    public int getItemCount() {
        return fosolKatarDhap.size();
    }


    static class ListFosolKatarDhap_ViewHolder extends RecyclerView.ViewHolder {
        ImageView diseaseImage;
        TextView diseaseTitle;
        CardView listItemCardView;


        public ListFosolKatarDhap_ViewHolder(@NonNull View itemView) {
            super(itemView);
